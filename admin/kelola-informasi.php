...
if (($_POST['action'] ?? '') === 'add') {
  ...
  if ($kategori === 'lainnya') {
    ...
    if (!in_array($tipe, ['himbauan', 'pengumuman', 'layanan'], true))
      $tipe = 'himbauan';
  } else {
    if ($nama === '')
      $err = "Nama wajib diisi.";
    if ($tombol_text === '')
      $tombol_text = ($kategori === 'dpo') ? 'Laporkan' : 'Hubungi';
  }
...
<?php require __DIR__ . "/sidebar.php"; ?>
  <div class="container">
    ...
    <?php if ($flash): ?>
      <div class="msg ok"><?= h($flash) ?></div> <?php endif; ?>
    <?php if ($err): ?>
      <div class="msg bad"><?= h($err) ?></div> <?php endif; ?>

    <div class="grid">
      <div class="card">
        <h3 style="margin-top:0; margin-bottom:8px;">Input Data Baru</h3>
        <p style="font-size:13px; color:var(--text-muted); margin-bottom:20px;">Silakan pilih kategori terlebih dahulu
          untuk menyesuaikan formulir.</p>
        ...
        <form method="POST" enctype="multipart/form-data">
          ...
          <div
            style="display:grid; grid-template-columns: 100px 1fr; gap: 20px; align-items: center; background: #f8fafc; padding: 15px; border-radius: 10px; margin-bottom: 20px;">
          ...
          <div style="overflow-x:auto;">
            <table>
              ...
              <tbody>
                <?php if (count($list) === 0): ?>
                  <tr>
                    <td colspan="5" style="text-align:center; padding:40px; color:var(--text-muted);">Belum ada data
                      informasi.</td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($list as $it):
                    ...
                    <tr>
                      ...
                      <td>
                        <span
                          class="pill <?= $isActive ? 'on' : 'off' ?>"><?= $isActive ? '● AKTIF' : '● NONAKTIF' ?></span>
                        <div style="margin-top:8px;">
                          <button type="button" class="btn btn-action-sm"
                            onclick="submitToggle(<?= $id ?>, <?= $isActive ? 0 : 1 ?>)">
                            <?= $isActive ? 'Matikan' : 'Hidupkan' ?>
                          </button>
                        </div>
                      </td>
                      <td>
                        <input type="number" name="urutan[<?= $id ?>]" value="<?= (int) $it['urutan'] ?>"
                          style="padding: 6px; text-align:center;">
                      </td>
                      <td>
                        <button type="button" class="btn danger btn-action-sm"
                          onclick="confirmDelete(<?= $id ?>)">Hapus</button>
                      </td>
                    </tr>
                  <?php endforeach; ?>
                <?php endif; ?>
  ...
  <form id="toggleForm" method="POST"><input type="hidden" name="action" value="toggle"><input type="hidden" name="id"
      id="toggleId"><input type="hidden" name="to" id="toggleTo"></form>
  <form id="deleteForm" method="POST"><input type="hidden" name="action" value="delete"><input type="hidden" name="id"
      id="deleteId"></form>
