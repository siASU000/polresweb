...
try {
    ...
    if ($mode === 'create' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!$stmt->execute())
            throw new RuntimeException("Gagal Simpan: " . $stmt->error);
    ...
    if ($mode === 'edit' && $id > 0 && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!$stmt->execute())
            throw new RuntimeException("Gagal Update: " . $stmt->error);
    ...
    if ($mode === 'delete' && $id > 0) {
        ...
        if ($q && $row = $q->fetch_assoc()) {
            if ($row['gambar'])
                @unlink($beritaUploadDir . DIRECTORY_SEPARATOR . $row['gambar']);
        }
...
<head>
    ...
    <script src="https://cdn.tiny.cloud/1/qxoa1nvib16ymjts7my24crtxmphz7aab621ez64j0gxxfub/tinymce/6/tinymce.min.js"
...
<body>
    ...
    <?php if ($flashOk): ?>
        <div class="alert alert-ok"><?= h($flashOk) ?></div> <?php endif; ?>
    <?php if ($flashError): ?>
        <div class="alert alert-err"><strong>Error:</strong> <?= h($flashError) ?></div> <?php endif; ?>
            ...
            <table class="table">
                ...
                <tbody>
                    <?php if (empty($beritaList)): ?>
                        <tr>
                            <td colspan="7" style="text-align:center; padding: 30px; color:#999;">Belum ada berita yang
                                ditambahkan.</td>
                        </tr>
                    <?php endif; ?>
                    ...
                    <?php foreach ($beritaList as $b): ?>
                        <tr>
                            ...
                            <td>
                                <a href="?mode=edit&id=<?= $b['id'] ?>" class="btn"
                                    style="padding: 6px 12px; font-size:12px;">Edit</a>
                                <a href="?mode=delete&id=<?= $b['id'] ?>" class="btn btn-danger"
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="grid grid-1" style="margin-bottom: 20px;">
                    <div>
                        ...
                        <input type="text" name="judul" value="<?= h($beritaEdit['judul'] ?? '') ?>" required
                        ...
                        <div id="kat_box">
                            ...
                            <select name="kategori">
                                ...
                                <?php foreach ($KATEGORI_LIST as $kat): ?>
                                    <option value="<?= $kat ?>" <?= ($beritaEdit['kategori'] ?? '') === $kat ? 'selected' : '' ?>>
                                        <?= $kat ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea id="isi_editor" name="isi"
                            placeholder="Tulis isi berita lengkap..."><?= h($beritaEdit['isi'] ?? '') ?></textarea>
                    </div>

                    <div>
                        <label>Meta Description (SEO) <small class="text-muted">— Opsional, maks 160 karakter. Jika kosong
                                akan otomatis diambil dari isi berita.</small></label>
                        <input type="text" name="meta_description" value="<?= h($beritaEdit['meta_description'] ?? '') ?>"
                            placeholder="Deskripsi singkat untuk mesin pencari..." maxlength="160">
                    </div>

                    <div class="grid">
                        <div>
                            <label>Tanggal</label>
                            <input type="date" name="tanggal" value="<?= $beritaEdit['tanggal'] ?? date('Y-m-d') ?>"
                                required>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <?php if (!empty($beritaEdit['gambar'])): ?>
                            <div
                                style="background: #f8f8f8; padding: 10px; border-radius: 8px; display: flex; align-items: center; gap: 15px; margin-bottom: 10px;">
