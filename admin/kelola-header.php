...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_settings') {
  ...
  $stmt = $conn->prepare("UPDATE header_settings SET logo_path=?, search_enabled=?, call_center_label=?, call_center_wa=? WHERE id=1");
  $stmt->bind_param("siss", $logo, $search, $label, $wa);
  $stmt->execute();
  $stmt->close();
...
<head>
  ...
  <style>
    ...
    .btn-ok{background:#0b5ed7;color:#fff}
        ...
        <div class="row">
          ...
          <div style="display:flex;align-items:end;gap:10px">
            <label style="display:flex;align-items:center;gap:8px;font-weight:600">
              <input type="checkbox" name="search_enabled" <?= ((int)$hs['search_enabled']===1)?'checked':'' ?> style="width:auto">
              Aktifkan Search
            </label>
          </div>
        </div>
    ...
    <div class="card">
      ...
      <table>
        ...
        <tbody>
            ...
            <tr>
              ...
              <td style="display:flex;gap:8px;flex-wrap:wrap">
                <form method="post">
                  ...
                  <button class="btn btn-ok" type="submit"><?= ((int)$m['is_active']===1)?'Nonaktifkan':'Aktifkan' ?></button>
                </form>
          ...
          <?php endforeach; ?>
          <?php if (empty($menus)): ?>
            <tr><td colspan="7">Belum ada menu.</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
